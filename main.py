...
class LegacyLocalStack(werkzeug.local.Local):
    def __init__(self):
        super().__init__()
        self.stack = []
    @property
...
# 2. Inject it into the flask module at runtime before extensions look for it
if not hasattr(flask, "_request_ctx_stack"):
    flask._request_ctx_stack = LegacyLocalStack()
if not hasattr(flask, "_app_ctx_stack"):
    flask._app_ctx_stack = LegacyLocalStack()
...
def get_gravatar_url(email, size=100, rating='g', default='retro'):
...
def inject_gravatar():
    return dict(gravatar=get_gravatar_url)
...
#     return render_template("contact.html", msg_sent=False)True
#
#
